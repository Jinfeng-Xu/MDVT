...
from common.sample_generator import samples_gen

class MGCN_VP(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MGCN_VP, self).__init__(config, dataset)

        # config for -VP
        self.k = config['k']
        self.s_weight = config['s_weight']
        self.top_k_pos_samples = None
        self.top_k_neg_samples = None
        self.flag = False
    ...
    def calculate_loss(self, interaction):
        ...
        # VP added
        if self.flag:
            self.top_k_pos_samples, self.top_k_neg_samples = samples_gen(ua_embeddings, ia_embeddings, self.k)
            s_pos_scores = 0
            s_neg_scores = 0
            for idx in range(self.k):
                s_pos_scores = s_pos_scores + torch.sum(ua_embeddings * ia_embeddings[self.top_k_pos_samples[:, idx]],
                                                        dim=1)
                s_neg_scores = s_neg_scores + torch.sum(ua_embeddings * ia_embeddings[self.top_k_neg_samples[:, idx]],
                                                        dim=1)
            s_pos_scores = s_pos_scores / self.k
            s_neg_scores = s_neg_scores / self.k
            s_loss_value = -torch.mean(torch.log2(torch.sigmoid(s_pos_scores - s_neg_scores)))
            s_loss_value = self.s_weight * s_loss_value
            return (1-self.s_weight) * (batch_mf_loss + batch_emb_loss + batch_reg_loss + self.cl_loss * cl_loss) + s_loss_value
