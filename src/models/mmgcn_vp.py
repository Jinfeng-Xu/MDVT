...
class MMGCN_VP(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MMGCN_VP, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        # VP added
        if self.flag:
            self.top_k_pos_samples, self.top_k_neg_samples = samples_gen(user_score, item_score, self.k)
            s_pos_scores = 0
            s_neg_scores = 0
            for idx in range(self.k):
                s_pos_scores = s_pos_scores + torch.sum(user_score * item_score[self.top_k_pos_samples[:, idx]],
                                                        dim=1)
                s_neg_scores = s_neg_scores + torch.sum(user_score * item_score[self.top_k_neg_samples[:, idx]],
                                                        dim=1)
            ...
            return (1-self.s_weight) * (loss + reg_loss) + s_loss_value
