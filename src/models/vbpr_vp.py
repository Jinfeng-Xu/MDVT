...
class VBPR_VP(GeneralRecommender):
    r"""BPR is a basic matrix factorization model that be trained in the pairwise way.
    """
    def __init__(self, config, dataloader):
        super(VBPR_VP, self).__init__(config, dataloader)
    ...
    def calculate_loss(self, interaction):
        ...
        # VP added
        if self.flag:
            self.top_k_pos_samples, self.top_k_neg_samples = samples_gen(user_embeddings, item_embeddings, self.k)
            s_pos_scores = 0
            s_neg_scores = 0
            for idx in range(self.k):
                s_pos_scores = s_pos_scores + torch.sum(user_embeddings * item_embeddings[self.top_k_pos_samples[:, idx]], dim=1)
                s_neg_scores = s_neg_scores + torch.sum(user_embeddings * item_embeddings[self.top_k_neg_samples[:, idx]], dim=1)
            ...
            return (1-self.s_weight) * loss + s_loss_value
        return loss
