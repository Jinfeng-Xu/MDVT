...
class SLMRec_VP(GeneralRecommender):
    def __init__(self, config, dataset):
        super(SLMRec_VP, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.flag:
            s_loss = self.s_loss()
            return (1-self.s_weight) * (main_loss + self.config['ssl_alpha'] * ssl_loss) + s_loss
    ...
    def s_loss(self):
        ...
        self.top_k_pos_samples, self.top_k_neg_samples = samples_gen(all_users, all_items, self.k)
        s_pos_scores = 0
        s_neg_scores = 0
        for idx in range(self.k):
            s_pos_scores = s_pos_scores + torch.sum(all_users * all_items[self.top_k_pos_samples[:, idx]],
                                                    dim=1)
            s_neg_scores = s_neg_scores + torch.sum(all_users * all_items[self.top_k_neg_samples[:, idx]],
                                                    dim=1)
