...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False, vp='0'):
        ...
        # 0: None
        # 1: Virtue Pairs
        # 2: Dynamic Virtue Pairs
        self.vp = vp
        if self.vp == '0':
            self.logger.info('=======Virtue Pairs Will be Added=======')
        elif self.vp == '1':
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        if self.vp == '1' and epoch_idx >= self.warm_up and not self.model.flag:
            self.model.flag = True
            self.logger.info('=======Virtue Pairs Will be Added=======')

        # dynamic calculation started before few epochs training
        elif self.vp == '2':
            if epoch_idx >= 3:
                if not self.model.flag:
                    if ((self.pre_loss - total_loss) / total_loss) < self.threshold:
                        self.model.flag = True
                        self.logger.info('=======Virtue Pairs Will be Added=======')
                    self.pre_loss = total_loss
            else:
                self.pre_loss = total_loss
