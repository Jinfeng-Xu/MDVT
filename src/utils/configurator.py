...
class Config(object):
    ...
    def __init__(self, model=None, dataset=None, config_dict=None, mg=False, vp='0'):
        """
        Args:
        ...
        # model type
        self.final_config_dict = self._load_dataset_model_config(config_dict, mg, vp)
        # config in cmd and main.py are latest
        self.final_config_dict.update(config_dict)
        self._set_default_parameters()
        self._init_device()

    def _load_dataset_model_config(self, config_dict, mg, vp):
        ...
        if vp == '1':
            file_list.append(os.path.join(cur_dir, 'vp.yaml'))
        elif vp == '2':
            file_list.append(os.path.join(cur_dir, 'vpd.yaml'))
        hyper_parameters = []
